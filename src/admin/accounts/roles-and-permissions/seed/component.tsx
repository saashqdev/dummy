  ...
  return (
    <div className="space-y-4">
      <InputGroup title={'Missing roles & permissions'}>
        <div className="space-y-2">
          ...
          <div className="flex justify-end">
            <LoadingButton
              isLoading={pending}
              disabled={!getUserHasPermission(adminData, 'admin.roles.create')}
              onClick={onSeed}
            >
              Seed default: {data.roles.missing.length}/{data.roles.all.length} roles and{' '}
              {data.permissions.missing.length}/{data.permissions.all.length} permissions
            </LoadingButton>
          </div>
        </div>
      </InputGroup>
      ...
      <InfoBanner title={'Info'}>
        Use this page after you have added new roles and/or permissions on the file:{' '}
        <code>app/modules/permissions/data/DefaultRoles.ts</code>
      </InfoBanner>
    </div>
  )
}
