    ...
    <nav className={clsx("not-prose flex truncate", className)} aria-label="Breadcrumb">
      <ol className="flex items-center space-x-1">
        {home && (
          <li>
            <div>
              <Link href={home.length > 0 ? home : "/"} className="text-gray-300 hover:text-gray-400">
          ...
          .map((item, idx) => (
            <li key={item.title}>
              <div className="flex items-center">
                ...
                {item.routePath ? (
                  <Link href={item.routePath} className={clsx("select-none text-sm font-normal text-gray-400 hover:text-gray-600", home && "ml-1")}>
                    {item.title}
                  </Link>
                ) : (
