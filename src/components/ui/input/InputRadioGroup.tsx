...
export default function InputRadioGroup({ name, title, value, options, onChange, className, required, disabled }: Props) {
  return (
    <div className={clsx(className, "w-full flex-grow")}>
      ...
      <RadioGroup disabled={disabled} defaultValue={value} value={value} onChange={(e) => (onChange ? onChange(e) : {})} className="mt-1">
            ...
            <RadioGroup.Option
              ...
              className={({ active, checked }) =>
                clsx(
                  ...
                  checked ? "border-transparent bg-primary text-primary-foreground hover:bg-primary/90" : "",
