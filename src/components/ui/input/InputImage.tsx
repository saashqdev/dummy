...
export default function InputImage(props: Props) {
  ...
  const [showUploadImage, setShowUploadImage] = useState(
    (props.opened && (!value || defaultValue)) ?? false,
  )
  ...
  const imageClassName = clsx(
    ...
    size === 'large' && 'h-32',
    square &&
      clsx(size === 'small' && 'w-8', size === 'medium' && 'w-16', size === 'large' && 'w-32'),
  )
  ...
  return (
    <div className={clsx('space-y-1', props.className)}>
      ...
      <label
        htmlFor={props.name}
        className="flex items-baseline justify-between space-x-2 truncate text-xs font-medium"
      >
            ...
            {image.startsWith('<svg') ? (
              <div
                dangerouslySetInnerHTML={{
                  __html: image.replace('<svg', `<svg class='${imageClassName}'`) ?? '',
                }}
              />
            ) : (
              <Image className={imageClassName} src={image || ''} alt={props.title || ''} />
            )}
          </div>
        </div>
      )}
