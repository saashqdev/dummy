...
export default function CurrentSubscriptionButton({ mySubscription }: { mySubscription: TenantSubscriptionWithDetailsDto | null }) {
  const params = useParams();
  const { t } = useTranslation();

  function getSubscriptionProducts() {
    return mySubscription?.products.map((f) => t(f.subscriptionProduct.title)) ?? [];
  }
