    ...
    const items = tenants
      .map((f) => {
        ...
        return {
          ...
          img: f.tenant.icon ? (
            <Image
              className="inline-block h-6 w-6 shrink-0 rounded-md bg-gray-700 shadow-sm"
              src={f.tenant.icon}
              alt={f.tenant.name}
            />
          ) : (
            <span className="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-primary">
              <span className="text-theme-200 text-xs font-medium leading-none">
                {f.tenant?.name.substring(0, 1)}
              </span>
            </span>
          ),
