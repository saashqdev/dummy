...
export const signInAction = publicClient
  ...
  .action(async ({ clientInput }) => {
    ...
    if (user) {
      // finding user tenants and redirecting user to first tenant, last resort redirecting with their user-name
      const tenants = user?.tenants ?? []
      const tenantSlug = typeof tenants?.[0]?.tenant === 'object' ? tenants?.[0]?.tenant?.slug : ''
