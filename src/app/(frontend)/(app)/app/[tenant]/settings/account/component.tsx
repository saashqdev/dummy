...
import { actionAppSettingsAccount } from '../../actions'
...
export default function () {
  ...
  const [actionData, action, pending] = useActionState(
    (state: any, payload: FormData) => actionAppSettingsAccount(payload),
  ...
  function deleteAccount() {
    confirmDelete.current?.show(
      t('settings.danger.confirmDeleteTenant'),
      t('shared.confirm'),
      t('shared.cancel'),
  ...
  return (
    <IndexPageLayout>
      <SettingSection
        title={t('settings.tenant.general')}
        description={t('settings.tenant.generalDescription')}
      >
      ...
      >
        <div className="mt-12 md:col-span-2 md:mt-0">
          <div>
            ...
            <div>
              <ButtonPrimary
                disabled={!getUserHasPermission(appData, 'app.settings.account.delete')}
                destructive={true}
