...
export async function createRole(data: {
  ...
  const dbData = {
    ...
    type: data.type,
    assign_to_new_users: data.assignToNewUsers,
    is_default: data.isDefault,
  }
  return await db.role.create(dbData).then((item) => {
...
export async function updateRole(
  id: string,
  data: { name: string; description: string; type: 'admin' | 'app'; assignToNewUsers: boolean },
) {
  ...
  await db.role.update(id, dbData).then(() => {
    clearCacheKey(`roles:${data.type}`)
