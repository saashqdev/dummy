...
export async function getUserRoleInAdmin(userId: string, roleName: string): Promise<UserRoleModel | null> {
...
export async function createUserRole(data: { userId: string; roleId: string; tenantId: string | null }): Promise<string> {
...
export async function createUserRoles(userId: string, roles: { id: string; tenantId: string | null }[]): Promise<void> {
  const uniqueTenantIds = [...new Set(roles.map((role) => role.tenantId))];
  await db.userRole.createMany(userId, roles).then(() => {
...
export async function deleteUserRole({ userId, roleId, tenantId }: { userId: string; roleId: string; tenantId: string | null }): Promise<void> {
