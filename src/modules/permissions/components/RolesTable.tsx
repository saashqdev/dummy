  ...
  useEffect(() => {
      ...
      setActions([
        {
          ...
          onClickRoute: (_: any, item: any) =>
            `/admin/accounts/roles-and-permissions/roles/${item.id}`,
        },
      ])
    }
    ...
    const headers: RowHeaderDisplayDto<RoleWithPermissionsAndUsersDto>[] = [
      ...
      },
      {
        ...
        value: (i) => (
          <div className="w-40 truncate">
            {tenantId ? (
              <ButtonTertiary onClick={() => setSelectedRow(i)}>
                View {i.permissions.length} permissions
              </ButtonTertiary>
            ) : (
              <div>
                {i.permissions.length}{' '}
                <span className="max-w-sm text-xs italic text-gray-400">
                  ({i.permissions.map((f) => f.permission.name).join(', ')})
                </span>{' '}
              </div>
            )}
          </div>
      ...
      },
      {
        ...
        value: (i) => i.users.filter((f) => f.tenant_id === tenantId).length,
      },
    ]
    if (!tenantId) {
      headers.push({
