  ...
  return (
    <div className="relative overflow-hidden py-8">
      <div className="mx-auto max-w-5xl space-y-8 px-4 text-center sm:max-w-3xl sm:px-6 lg:max-w-5xl lg:px-8">
        <div>
          {item.topText && (
            <h2 className="text-theme-600 text-base font-semibold uppercase tracking-wider">
              {item.topText}
            </h2>
          )}
          {item.headline && (
...
function CustomCarousel({ items, size = 'full' }: CarouselProps) {
  ...
  const [currentItem, setCurrentItem] = useState<
    { type?: string; title?: string; src: string } | undefined
  >(undefined)
              ...
              <Card className="overflow-hidden rounded-lg">
                {(!item.type || item.type === 'image') && (
                  <Image
                    ...
                    loading="lazy"
                    className={clsx(
                      'min-h-full w-full object-cover md:h-auto',
                      size === 'sm' && 'h-48',
                      size === 'md' && 'h-64',
                      size === 'lg' && 'h-96',
                    )}
